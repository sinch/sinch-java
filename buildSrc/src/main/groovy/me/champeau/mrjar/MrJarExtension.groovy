...
abstract class MrJarExtension {
    ...
    void addLanguageVersion(int version) {
        ...
        // and what you're likely to want to do, is to provide a JDK > 8 specific class, which depends on common
        ...
        // We want to avoid recompiling all those classes, so we're just saying that the Java > 8 specific classes
        // "depend on" the main ones (same for shared test classes e.g. extensions).
        ...
        dependencies."java${version}TestImplementation"(sharedTestSourceSet.output.classesDirs)

        configurations.findByName("java${version}Implementation").extendsFrom configurations.findByName("implementation")
        configurations.findByName("java${version}TestImplementation").extendsFrom configurations.findByName("java${version}Implementation")
        configurations.findByName("java${version}TestImplementation").extendsFrom configurations.findByName("testImplementation")
        ...
        tasks.named("compileJava${version}TestJava") {
            it.javaCompiler = targetCompiler
        }
